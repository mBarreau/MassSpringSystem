import numpy as np
import cvxpy as cp
from lambda_min import lambda_min, lambda_min_cl
from A0Hurwitz import A0Hurwitz, A0Hurwitz_cl


def local_LMI_cl(A, B, C, model, vref, Kp):
    m, g, kv, k = model.m, model.g, model.kv, model.k
    muS, muC, vs = model.muS, model.muC, model.vs

    vref1, vref2 = A0Hurwitz_cl(model, Kp)
    if vref1 <= vref <= vref2:
        return None
    ...
    Gamma = -2 * m * g * (muS - muC) * (vref / vs**2) * np.exp(-vref**2 / vs**2)
    Gamma_cl = Gamma - Kp
    A0 = A  
    ...
    for rl in rlList:
        lam_cl = lambda_min_cl(model, vref, rl, Kp)

        ThetaLocal = cp.bmat([
            [A0.T @ Pl + Pl @ A0 - 2*tau*Gamma_cl*(Gamma_cl + lam_cl)*(C.T @ C),
             Pl @ B - tau*(2*Gamma_cl + lam_cl)*C.T],
            [(Pl @ B - tau*(2*Gamma_cl + lam_cl)*C.T).T,
...
def local_LMI(A,B,C, model, vref, Kp):
    m, g, kv, k = model.m, model.g, model.kv, model.k
    muS, muC, vs = model.muS, model.muC, model.vs
