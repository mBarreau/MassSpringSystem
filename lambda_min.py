...
def lambda_min_cl(physics, vref, rl, Kp=0.0):
    m = physics.m
    g = physics.g
    muS = physics.muS
    muC = physics.muC
    ...
    def phi_cl(eps1):
        return phi(eps1) - Kp * eps1

    def partial_phi_cl(eps1):
        return partial_phi(eps1) - Kp

    if max(abs(v_star_minus), abs(v_star_plus)) <= rl:
        lambda_min_val = max(-partial_phi_cl(v_star_minus), -partial_phi_cl(v_star_plus))
    else:
        lambda_min_val = -min(-phi_cl(-rl) / rl, phi_cl(rl) / rl)
